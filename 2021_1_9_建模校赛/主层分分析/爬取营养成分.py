...
# 爬取单独营养成分表
def get_nutrition(url,kd,link_dict,i):
    url = url
    keyword = kd
    link_dict = link_dict
    response = requests.get(url)
    ...
    # 创建对应的csv文件
    name = link_dict[url]
    csv_write.writerow([i,name,value[0],value[1],value[2],value[3],value[4],value[5],value[6],value[7],value[8],value[9],
    ...
    with  open("所有食物相关信息整合.csv", mode='a+', encoding='utf-8-sig', newline="") as f:
        ...
        try:
            for i in range(len(keywords)):
                get_nutrition(urls[i],keywords[i],link_dict,i)
